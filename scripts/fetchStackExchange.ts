...
export const fetchQuestions = async (): Promise<string> => {
	...
	// https://api.stackexchange.com/docs/questions#filter=!1ZFzxYL2ELUKqhj_*4sTn2BvPAtLgsEA.&run=true
	...
	for (const question of questions.items.filter((q) => q.is_answered)) {
		...
		if (!question.accepted_answer_id) {
			console.log(
				"Skipping #%s '%s' because it does not have an accepted answer",
